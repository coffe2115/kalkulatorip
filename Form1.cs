...
namespace IpKalkulator
{
    public partial class Form1 : Form
    {
        ...
        private void IPAddressText_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
        ...
        private void CalculateIP(object sender, EventArgs e)
        {
           ...
           ipString = rgx.Replace(ipString, ".");
            ...
            byte[] reveresedMask = maskAddress.GetAddressBytes();
            byte[] network = new byte[4];
            byte[] broadcast = new byte[4];
            for (int i = 0; i < 4; i++)
            {
                network[i] = (byte)(ip[i] & mask[i]);
                reveresedMask[i] = (byte)~mask[i];
                broadcast[i] = (byte)(network[i] | reveresedMask[i]);
            }
